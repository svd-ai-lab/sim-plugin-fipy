...
hatchling picks it up via ``packages = ["src/sim_plugin_fipy"]``
...
def test_wheel_contains_skills(tmp_path: Path) -> None:
    ...
    wheels = list(out_dir.glob("sim_plugin_fipy-*.whl"))
    ...
    required = {
        "sim_plugin_fipy/__init__.py",
        "sim_plugin_fipy/driver.py",
        "sim_plugin_fipy/_skills/fipy/SKILL.md",
    }
